...
type FormValues = Readonly<{ title: string; slug: string; description: string; link: string; sortOrder: string; removeCoverImage: boolean; coverImage: File | null; mediaAssets: File[] }>;
...
export const BrandEditPage: React.FC = () => {
  ...
  const form = useEntityForm<FormValues, SubmitValues>({
    schema,
    initialValues: { title: "", slug: "", description: "", link: "", sortOrder: "", removeCoverImage: false, coverImage: null, mediaAssets: [] },
  ...
  const coverImageSrc = React.useMemo(() => {
    if (form.values.coverImage instanceof File) return URL.createObjectURL(form.values.coverImage);
    if (form.values.removeCoverImage) return "";
    return (
      (get.data as { coverImage?: string; coverImageUrl?: string; image?: string } | undefined)?.coverImage ??
      (get.data as { coverImage?: string; coverImageUrl?: string; image?: string } | undefined)?.coverImageUrl ??
      (get.data as { coverImage?: string; coverImageUrl?: string; image?: string } | undefined)?.image ??
      ""
    );
  }, [form.values.coverImage, form.values.removeCoverImage, get.data]);
  React.useEffect(() => {
    return () => {
      if (form.values.coverImage instanceof File && coverImageSrc) URL.revokeObjectURL(coverImageSrc);
    };
  ...
  React.useEffect(() => {
    ...
    const existingColor =
      (get.data as { dominantColor?: string; color?: string; bgcolor?: string } | undefined)?.dominantColor ??
      (get.data as { dominantColor?: string; color?: string; bgcolor?: string } | undefined)?.color ??
      (get.data as { dominantColor?: string; color?: string; bgcolor?: string } | undefined)?.bgcolor ??
      "";
  ...
  if (get.isError) return <div>Failed to load</div>;
      ...
      <form onSubmit={(e) => { e.preventDefault(); void form.submit(); }} style={{ display: "grid", gap: 10 }}>
        <EntityFormRenderer
          ...
          onRemoveExistingPreview={(name, index) => {
            ...
            if (name === "mediaAssets") {
              const assets = (get.data?.mediaAssets ?? []) as ReadonlyArray<unknown>;
              const asset = assets[index] as { id?: string } | undefined;
              if (asset?.id) {
                setRemovedMediaAssetIds((prev) => (prev.includes(asset.id as string) ? prev : [...prev, asset.id as string]));
              }
            }
          }}
          filePreviewUrls={{
            coverImage:
              form.values.removeCoverImage
                ? null
                : ((get.data as { coverImage?: string; coverImageUrl?: string; image?: string } | undefined)?.coverImage ??
                  (get.data as { coverImage?: string; coverImageUrl?: string; image?: string } | undefined)?.coverImageUrl ??
                  (get.data as { coverImage?: string; coverImageUrl?: string; image?: string } | undefined)?.image ??
                  null),
            mediaAssets: ((get.data?.mediaAssets ?? []) as ReadonlyArray<unknown>).filter((a) => {
